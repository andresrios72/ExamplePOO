...
public class AgenteDos : PortadorJugable
{
    ...
    public override void UsarHabilidad(int index, Portador objetivo)
    {
        ...
        if (mana.ValorActual >= habilidad.costo && habilidad.PuedeUsarse(Time.time))
        {
            mana.AfectarEstidistica(-habilidad.costo);
            habilidad.Ejecutar(objetivo);
            Debug.Log($"{nombre} usó {habilidad.nombre} con maná (-{habilidad.costo}).");
        }
        else
        {
            Debug.Log($"{nombre} no puede usar {habilidad.nombre}: maná insuficiente o en cooldown.");
        }
    }
    private void Update()
    {
